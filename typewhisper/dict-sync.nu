...
def is-list-or-table [value] {
    ($value | describe) =~ '^(list|table)'
}

def parse-dictionary [data, source: string] {
    ...
    if not (is-list-or-table $corrections) {
        error make {msg: $"($source): \"corrections\" must be an array of \{original, replacement, caseSensitive\}"}
    }
