# macOS system configuration for ryoppippi
{
  pkgs,
  inputs,
  hostName,
...
in
{
  ...
  # Apply overlays for Home Manager packages
  nixpkgs.overlays = [
    (_final: _prev: {
      _llm-agents = inputs.llm-agents;
      _claude-code-overlay = inputs.claude-code-overlay;
    })
  ...
  homebrew = {
    ...
    # Some casks are managed via brew-nix (see home configuration packages):
