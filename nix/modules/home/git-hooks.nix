...
let
  # Create treefmt wrapper with the same configuration as flake.nix
...
in
{
  ...
  home.activation.installGitHooks = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        ...
        if [ -d "$DOTFILES_DIR/.git" ]; then
          ...
          # Create the pre-commit hook script with rebase/merge skip logic
    ...
    if [ -n "$STAGED_FILES" ]; then
      ${treefmtWrapper}/bin/treefmt --no-cache $STAGED_FILES
