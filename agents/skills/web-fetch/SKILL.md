...
Delegate the reading, not only the fetching: hand the whole task to a `codex exec` subagent, which searches on the Codex subscription and returns a digest, so whole pages and JSON payloads never enter this context — see the `ask-codex` skill. Two things stay here: `exa`, whose 1Password key no sandbox mode reaches, and anything wanted verbatim, such as the exa docs pages above, where a digest rewrites the exact names you came for.
