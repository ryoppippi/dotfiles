...
local function setup()
  ...
  if match !=# ''
    ...
    augroup dps-dial
      ...
      autocmd FileType python let b:dps_dial_augends = ['decimal', {'kind': 'constant', 'opts': {'elements': ['True', 'False']}}, {'kind': 'case', 'opts': {'cases': ['camelCase', 'snake_case'], 'cyclic': v:true}}]
      autocmd FileType typescript let b:dps_dial_augends = g:dps_dial#augends +  ['decimal', {'kind': 'constant', 'opts': {'elements': ['const', 'let'],'cyclic': v:true,'word': v:true}}]
...
require("utils.plugin").load_denops_plugin("dial", setup)
